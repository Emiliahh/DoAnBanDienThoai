...
const SendMailVerifyAccount = require('../utils/SendMailVerify');
...
class controllerUsers {
    ...
    async login(req, res) {
        ...
        await modelOtp.create({ email: user.email, otp, type: 'verify' });
        ...
        await SendMailVerifyAccount(user.email, otp);
        new OK({
    ...
    async verifyAccount(req, res) {
        ...
        await createApiKey(user._id);
        const tokenLogin = await createToken({ id: user._id });
        const refreshToken = await createRefreshToken({ id: user._id });
        ...
        new OK({
            message: 'Đăng nhập thành công',
            metadata: { tokenLogin, refreshToken },
        }).send(res);
    }
